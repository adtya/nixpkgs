...
#!nix-shell -I nixpkgs=channel:nixpkgs-unstable -i python3 -p "python3.withPackages (ps: with ps; [ aiohttp packaging ])" -p git nurl nodePackages.pyright isort ruff
...
class File:
    ...
    def substitute(self, key: str, old_value: str, new_value: str) -> None:
        old_line = self.get_exact_match(key, old_value)
...
class Nurl:
    @classmethod
    async def prefetch(cls, url: str, version: str) -> str:
...
class HomeAssistant:
    ...
    async def update_frontend(self, old_version: str, new_version: str) -> None:
        old_hash = str(await Nix.eval("home-assistant.python.pkgs.home-assistant-frontend.src.outputHash"))
        new_hash = await Nurl.prefetch("https://github.com/home-assistant/frontend/", new_version)
...
async def main():
    ...
    async with aiohttp.ClientSession(headers=headers) as client:
        ...
        if core_latest > core_current:
            ...
            frontend_latest = parse_version(await hass.get_latest_frontend_version(str(core_latest)))
